...
public class WaitListService {
	...
	* @Date: 2015年4月16日
	...
	public boolean validateNum(WaitList waitList) {
		boolean flag = false;
		if(waitList==null) return flag;
		List<WaitList> list = new ArrayList<WaitList>();
		if(waitList.getpId()==null || waitList.getpId()==0){
			//一级
			list = getOneLev(waitList.getStyleId());
			Integer maxSize = Integer.valueOf(DictMan.getDictItem("d_sys_param", 3).getItemName());
			...
			Integer maxSize = Integer.valueOf(DictMan.getDictItem("d_sys_param", 4).getItemName());
			if(waitList.getId()!=null){//更新操作
				maxSize = maxSize+1;
			}
			if(list!=null && list.size() < maxSize){
